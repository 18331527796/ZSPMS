			...
			<div class="am-g tpl-amazeui-form">
				...
				<div class="am-u-sm-12 am-u-md-9">
					<form id="myform" name="myform" class="am-form am-form-horizontal"
						action="channelupdate.do" method="post" onsubmit="return chk()">
						<input type="hidden" name="id" value="${CHANNELBYID.id}">
						<div class="am-form-group">
							<label for="user-name" class="am-u-sm-3 am-form-label">栏目名
								/ ChannelName</label>
							<div class="am-u-sm-9">
								<input type="text" id="cname" value="${CHANNELBYID.cname}" name="cname"
						...
						<div class="am-form-group">
                                    ...
                                    <div class="am-u-sm-9">
                                      ...
                                      <select name="pid" style="width: 300px"id="pid">
                                   			<c:forEach items="${SHANGJI}" var="shangji">
                                   				
                                   				<c:if test="${shangji.id==CHANNELBYID.pid}">
                                   					<option value="${shangji.id}" selected="selected">${shangji.cname}</option>
                                   				</c:if>
                                   				<c:if test="${shangji.id!=CHANNELBYID.pid&&shangji.id!=CHANNELBYID.id}">
                                   					<c:if test="${shangji.isleaf!=1}">
                                   						<option value="${shangji.id}" >${shangji.cname}</option>
                                   					</c:if>
                                   				</c:if>
                                   			</c:forEach>
                                   				<c:if test="${CHANNELBYID.pid==0}"><option value="0" selected="selected">无</option></c:if>
                                   				<c:if test="${CHANNELBYID.pid!=0}"><option value="0" >无</option></c:if>
                                      </select>
                                        <small></small>
                                    </div>
                                </div>
						 ...
						 <div class="am-form-group">
							...
							<c:if test="${CHANNELBYID.isleaf==0}">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio"
									name="isleaf" value="0" checked="checked">否<small></small>
							</c:if>
							</div>
						...
						<div class="am-form-group">
							<label for="user-name" class="am-u-sm-3 am-form-label">等级
								/ Lev</label>
							<div class="am-u-sm-9">
								<input type="text" value="${CHANNELBYID.lev}" id="lev" name="lev"
									placeholder="等级   / Lev" onblur="chklev()"> <small>等级要求(必填0-9其一)<span id="levmsg"></span></small>
							</div>
						</div>

						<div class="am-form-group">
							<label for="user-email" class="am-u-sm-3 am-form-label">顺序
								/ Sort</label>
							<div class="am-u-sm-9">
								<input type="text" id="sort" name="sort" value="${CHANNELBYID.sort}"
									placeholder="输入你的电子邮件 / Email" onblur="chksort"> <small>顺序决定栏目在页面中出现的位置(必填0-9其一)<span id="sortmsg"></span></small>
							</div>
						</div>
